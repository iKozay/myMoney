...
//              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Expanded(flex:5,child: Text("Budget", style: TextStyle(color: Color(0xffffffff), fontSize: 26,),textAlign: TextAlign.left,)),
                Expanded(
                  child: IconButton(
              ...
              child: LayoutBuilder(
                builder: (context, constraint) {
                  return SingleChildScrollView(
                    child: Column(
                      children: List.generate(CategoriesModel.list.length, (index) => Padding(
                        ...
                        Padding(
                          ...
                          child: Row(
                            children: [
                              const Spacer(),
                              Text(CategoriesModel.list.elementAt(index).name, style:TextStyle(color: Color(0xffffffff,), fontSize: 18)),
                              const Spacer(flex: 4,),
                              Text((CategoriesModel.list.elementAt(index).percentage*100).toInt().toString()+"%", style:TextStyle(color: Color(0xffffffff), fontSize: 18)),
                              const Spacer(),
                            ],
                          ),
                        ),
  ...
  List<ChartData> getChartData(){
    ...
    for (var i = 0; i < CategoriesModel.list.length; i++) {
      chartData.add(ChartData(CategoriesModel.list[i].name, CategoriesModel.list[i].percentage*100));
    }
    return chartData;
  }
